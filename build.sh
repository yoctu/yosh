...
sudo apt-get update >&/dev/null ; sudo apt-get install -y apt-transport-https devscripts debianutils jq gridsite-clients ldap-utils uuid-runtime xmlsec1 xmlstarlet
...
sudo apt-get download yoctu-scripts
sudo dpkg --force-all -i yoctu-scripts*.deb
